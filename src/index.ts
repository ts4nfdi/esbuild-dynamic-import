      ...
      build.onLoad({ filter }, async args => {
        ...
        if (!value || value.fileContents !== fileContents) {
          ...
          value = {
            ...
            output: {
              contents,
              loader: fileExtension === '.json' ? 'json' : loader,
            },
          };
...
async function replaceImports(fileContents: string, resolveDir: string, config: DynamicImportConfig) {
  ...
  if (globImports.length > 0) {
    ...
    try {
            importFilePaths = (await Promise.all(filenameImportPromises))
                .flat()
                .filter(filePath => /\.(js|json)$/.test(filePath)); // Allow both .js and .json
        } catch (e) {
      console.error(e);
    }
